...
in \texttt{logreg}. The only exceptions are the model selection procedures provided by \texttt{logreg}.
The standard implementation cannot be applied to case-parent trio data, as the special structure of these data must
be taken into account. Therefore, the \texttt{R} package \texttt{trio} provides special permutation tests via a
function called \texttt{trio.permTest} that can be used to apply a null-model test with, for example, 20 permutations 
to the case-parent trio data in \texttt{trio.bin} by calling
\begin{verbatim}
   > trio.permTest(triolr.out, n.perm=20)
\end{verbatim}
and a conditional permutation test can be performed by
\begin{verbatim}
