...
{\Large Qing Li and Holger Schwender}
\vspace*{0.1cm}
\end{center}
%\doublespace
...
The \texttt{R} package \texttt{trio} provides functionalities relevant
...
Furthremore, \texttt{trio} contains functions for performing genotypic transmission
disequilibrium test (gTDT) for testing individual SNPs \citep{schaid1996} and
two-way interactions of SNPs \citep{cordell2002, cordell2004} whether the distributions
of their genotypes differ between the cases, i.e.\ the children, and the pseudo-controls
derived from the parents' genotypes.

Section \ref{trio} of this vignette is devoted to the steps relevant
...
Section \ref{simulation} of the vignette explains in more detail how to set
...
Section \ref{getLD} describes how the pairwise values of the typically used
LD measures $D^\prime$ and $r^2$ can be computed with the function \texttt{getLD()}
provided by \texttt{trio}, and how the $D^\prime$ values can be employed to
estimate haplotype blocks with the algorithm of \citet{gabriel2002}. The
resulting haplotype structure can then be used in the functions discussed in Section \ref{trio}.

In Section \ref{read}, it is shown how family-based data stored in a linkage/ped file can be read into
...
SNPs and two-way SNP interactions.
...
(the default), or in genotype format. Therefore, the \texttt{R} package \texttt{trio}
provides a function called \texttt{read.pedfile} for reading a linkage/ped file, in which
typically family-based data are stored, into \texttt{R} (for details, see Section \ref{testing}).
 
The first six columns in such a ped or linkage file identify the family structure of the data, and the
...
to be unrelated. This matrix is available by calling

<<>>=
data(LDdata)
@ 

and contains simulated genotype data for 10 LD blocks each consisting of 5 SNPs each typed on 500 subjects.
...
If the trio data is not already available in \texttt{R} as a data frame in ped format or a matrix in genotype format 
(see Section \ref{trio}), but in a ped file called, for example, "pedfile.ped" in the working directory of the current
\texttt{R} session (this file is actually not available in \texttt{trio}, we just assume that such a file exists in the working
directory), then this file can be read into \texttt{R} by calling

\begin{verbatim}
   > ped <- read.pedfile("pedfile.ped")
\end{verbatim}
If the arguments \texttt{coded} and \texttt{first.row} are not specified by the user, \texttt{read.pedfile()} automatically
...
(with extensions \texttt{.1} and \texttt{.2} to differ between the two alleles) to the respective columns of the read data frame. 
...
\end{verbatim}
The output of these functions just contains the matrix in genotype format, whereas \texttt{trio.check()} additionally contains information
...
If, for example, the data frame \texttt{trio.ped1} also used in the examples of Section \ref{trio} should be transformed into a
...
As mentioned in Section \ref{trio}, the \texttt{trio.gen1} is the genotype matrix corresponding to
...
\section{Testing SNPs and two-way SNP interactions}\label{testing}

A single SNP or a two-way interaction can be tested with a gTDT by employing the functions \texttt{tdt()}
...
By default, an additive effect is assumed. It is, however, also possible to consider a dominant effect
<<>>=
...
Thus, two conditional logistic regression models are fitted, one consisting of two dummy variables for each
...
also provide a simpler test, in which the values of the SNPs (either coding for an additive -- which is the default --
for a recessive, or a dominant model) are multiplied, and a conditional logistic regression is applied to this 
interaction.
...
The top three SNPs are shown by
...
Analogously, \texttt{trio} provides a function \texttt{colTDT2way()} for testing two-way interactions. Since this gTDT
assumes that the considered loci are unlinked, the testing might fail, i.e.\ the fitting of at least one of the two 
conditional logistic regressions might not work properly, if SNPs are in (strong) LD. (Another reason why the fitting
might not work properly is that the minor allele frequencies of both SNPs are small.) Therefore, \texttt{colTDT2way()}
