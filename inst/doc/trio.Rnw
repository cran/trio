...
disequilibrium tests (gTDTs) for testing whether the distributions of individual SNPs \citep{schaid1996} or
two-way interactions of SNPs \citep{cordell2002, cordell2004} 
differ between the cases, i.e.\ the children, and the pseudo-controls
derived from the parents' genotypes.

Furthremore, \texttt{trio} provides functionalities relevant
...
and for detecting LD blocks. In Section \ref{getLD}, it is described how these
pairwise values of the typically used LD measures $D^\prime$ and $r^2$ can be computed 
...
The first six columns in such a ped file, which is also called linkage file, 
...
\texttt{trio.gen1} (except that the first two columns of \texttt{trio.gen1} contain the family ID and the personal ID). 
...
or for a recessive, or for a dominant model) are multiplied, and a conditional logistic regression is applied to this 
interaction.
