...
Section \ref{getLD} shows how the pairwise values of the typically used
...
In Section \ref{read}, it is described how family-based data stored in a ped file can be read into
\texttt{R} and transformed into a format suitable for the application of the functions for performing the gTDTs,
whereas Section \ref{testing} contains examples for how these gTDT functions can be used to test individual 
...
provides a function called \texttt{read.pedfile} for reading a ped file, in which
...
\texttt{R} session, then this file can be read into \texttt{R} by calling
...
about Mendelian errors. Neither \texttt{ped2geno()} nor \texttt{read.pedfile()} check for Mendelian errors. Instead, such errors are removed
SNP-wise in the functions for performing genotypic TDTs.
