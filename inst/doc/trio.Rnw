...
SNPs, two-way SNP interactions, and gene-environment interactions, Section \ref{score} briefly shows how the allelic TDT and the 
score tests related to the genotypic TDTs can be used to test these factors.
...
In Section \ref{triolr}, it is shown how trio logic regression and trioFS
...
This is the recommended way to test the two-way interaction. \texttt{tdtGxG}, however,
...
<<>>=
tdtGxG(mat.test[,1], mat.test[,2], epistatic=FALSE)
@
...
might not work properly is that the minor allele frequencies of both SNPs are very small.) Therefore, \texttt{colGxG}
...
By default, the test statistic for the allelic TDT is adjusted for continuity by adding 1 to the difference in the denominator of
McNemar's test statistic, i.e.\
\begin{equation*}
a_\text{Cor}= \dfrac{(|b-c|-1)^2}{b+c}
\end{equation*}
is used as test statstic, where $b$ and $c$ are the off-diagonal elements of the 2x2-table summarizing the transmitted and untransmitted
alleles from heterozygous parents, i.e. $b$ is the number of heterozygous parents that transmitted the minor allele to their respective
children, and $c$ the number of heterozygous parents that transmitted the major allele. This correction can be avoided and
\begin{equation*}
a=\dfrac{(b-c)^2}{b+c}
\end{equation*}
can be used as test statistic by calling

<<>>=
a.out2 <- allelicTDT(mat.test, correct=FALSE)
a.out2
@
